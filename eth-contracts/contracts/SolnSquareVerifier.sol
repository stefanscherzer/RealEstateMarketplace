...
contract SolnSquareVerifier is ERC721Mintable {
    ...
    SquareVerifier private contractVerifier;
    ...
    function getSolutionKey(uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) internal pure returns(bytes32)
    {    
    ...
    function mintNFT(address to, uint256 tokenId, uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) public returns(uint256){
        ...
        require(solutionArray[key].solutionAddress == address(0), "Requires an unique solution!");
        require(solutionArray[key].minted == false, "Token already minted for this solution");
        require(contractVerifier.verifyTx(a,b,c,input), "Can't verify contract!");
        add(to, tokenId, key);
        super.mint(to, solutionArray[key].index, "tokenURI");
        solutionArray[key].minted = true;
    }
}
